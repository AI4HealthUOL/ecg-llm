...
class ModelEvaluator:
    def __init__(self, model_path, initial_model, test_file_path):
        logger.info("Initializing model evaluator...")
        self.is_initial_model = model_path == initial_model
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        if torch.cuda.is_available():
            # 70B models require special handling, it should be loaded in 4-bit, otherwise it cannot be loaded locally
            if "70" in model_path:
        ...
        for i, data_point in enumerate(self.test_data):
            ...
            if self.is_initial_model:
                question = self._form_prompt(
                    system_message, f"{data_point_question} Please answer shortly in one sentence without lists!"
                )
            else:
                question = self._form_prompt(system_message, data_point_question)
